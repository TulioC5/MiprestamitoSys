...
router.post("/eliminarUsuario", async (req, res) => {
    try {
      ...
      if (resultadoConsulta == true) {
        consulta = `INSERT INTO heroku_6f18d24c0ac41a5.tbl_bitacora_db(Tbl_Nombre, Accion, Registro_Despues, Usuario, Fecha) VALUES ('${tabla}', '${"actualizar"}', 'Id: ${Id_Usuario}, Estado: 2', '${payload.user}', CURRENT_TIMESTAMP)`;
        console.log(consulta);
        var resultadoConsulta = await realizarDml(consulta);
  ...
  router.post("/cambiarRol", async (req, res) => {
    try {
      ...
      console.log(resultadoConsulta);
      if (resultadoConsulta == true) {
        consulta = `INSERT INTO heroku_6f18d24c0ac41a5.tbl_bitacora_db(Tbl_Nombre, Accion, Registro_Despues, Usuario, Fecha) VALUES ('${tabla}', '${"actualizar"}', 'Id: ${Id_Usuario}, Id_Rol: ${Id_Rol}', '${payload.user}', CURRENT_TIMESTAMP)`;
        console.log(consulta);
        var resultadoConsulta = await realizarDml(consulta);
        res.status(200).send({ Ok: "Ok" });
      } else {
